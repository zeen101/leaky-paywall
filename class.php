...
if ( ! class_exists( 'Leaky_Paywall' ) ) {
	...
	class Leaky_Paywall {
		...
		function settings_page() {
			...
			} elseif ( $_GET['page'] == 'issuem-leaky-paywall' ) {
				$tab = 'general';
			} else {
				$tab = '';
			}
					...
					<form id="leaky-paywall" method="post" action="">
						...
						if ( in_array( $current_tab, $settings_tabs ) ) {
							...
							<h2 class="nav-tab-wrapper" style="margin-bottom: 10px;">
								...
								<a href="<?php echo admin_url( 'admin.php?page=issuem-leaky-paywall' ); ?>" class="nav-tab
													<?php
													if ( $current_tab == 'general' ) {
														?>
									 nav-tab-active<?php } ?>"><?php _e( 'General', 'leaky-paywall' ); ?></a>

								<a href="<?php echo admin_url( 'admin.php?page=issuem-leaky-paywall&tab=subscriptions' ); ?>" class="nav-tab
													<?php
													if ( $current_tab == 'subscriptions' ) {
														?>
									 nav-tab-active<?php } ?>"><?php _e( 'Restrictions & Levels', 'leaky-paywall' ); ?></a>

								<a href="<?php echo admin_url( 'admin.php?page=issuem-leaky-paywall&tab=payments' ); ?>" class="nav-tab
													<?php
													if ( $current_tab == 'payments' ) {
														?>
									 nav-tab-active<?php } ?>"><?php _e( 'Payments', 'leaky-paywall' ); ?></a>

								<a href="<?php echo admin_url( 'admin.php?page=issuem-leaky-paywall&tab=emails' ); ?>" class="nav-tab
													<?php
													if ( $current_tab == 'emails' ) {
														?>
									 nav-tab-active<?php } ?>"><?php _e( 'Emails', 'leaky-paywall' ); ?></a>

								<a href="<?php echo admin_url( 'admin.php?page=issuem-leaky-paywall&tab=licenses' ); ?>" class="nav-tab
													<?php
													if ( $current_tab == 'licenses' ) {
														?>
									 nav-tab-active<?php } ?>"><?php _e( 'Licenses', 'leaky-paywall' ); ?></a>

								<?php do_action( 'leaky_paywall_settings_tabs_links', $current_tab ); ?>

								<a href="<?php echo admin_url( 'admin.php?page=issuem-leaky-paywall&tab=help' ); ?>" class="nav-tab
													<?php
													if ( $current_tab == 'help' ) {
														?>
									 nav-tab-active<?php } ?>"><?php _e( 'Help', 'leaky-paywall' ); ?></a>
						...
						<?php
						if ( $current_tab == 'emails' ) : // email tab settings
							?>
						...
						<?php
						if ( $current_tab == 'payments' ) : // payments settings tab
							?>
						...
						<?php
						if ( $current_tab == 'subscriptions' ) : // subscriptions settings tab
							?>
							...
							<table id="leaky_paywall_default_restriction_options" class="form-table">
								...
								<tr class="restriction-options">
									...
									<td>
										...
										<?php
										if ( $this->check_for_caching() && $settings['enable_js_cookie_restrictions'] != 'on' ) {
											?>
