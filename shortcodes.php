	...
	function do_leaky_paywall_profile( $atts ) {
		...
		if ( is_user_logged_in() ) {
			...
			foreach( $sites as $site ) {
				...
				if ( 0 == strcasecmp('active', $status) && !leaky_paywall_user_has_access() ) {
					$status_name = 'Expired';
				} else {
			...
			$results .= '<div class="issuem-leaky-paywall-subscriber-info">';
				...
				foreach( $sites as $site ) {	
					...
					if ( 0 == strcasecmp( 'active', $status ) || 0 == strcasecmp( 'deactivated', $status ) ) {
						$payment_form = '';
	...
	leaky_paywall_show_error_messages( 'register' );
		...
		<div class="leaky-paywall-registration-payment-container">
			...
			<?php 
				...
				<?php if ( 'test' == leaky_paywall_get_current_mode() ) {
					?>
					</div>
				<?php } ?>
