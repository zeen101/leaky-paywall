...
class Leaky_Paywall_Payment_Gateway_Stripe_Checkout extends Leaky_Paywall_Payment_Gateway_Stripe {
	...
	public function process_confirmation() {
		if ( empty( $_GET['leaky-paywall-confirm'] ) && 'stripe_checkout' !== $_GET['leaky-paywall-confirm'] ) {
			return false;
		}

		$settings = get_leaky_paywall_settings();

		$this->email    = isset( $_POST['stripeEmail'] ) ? sanitize_email( wp_unslash( $_POST['stripeEmail'] ) ) : '';
		$this->level_id = isset( $_POST['custom'] ) ? sanitize_text_field( wp_unslash( $_POST['custom'] ) ) : '';
