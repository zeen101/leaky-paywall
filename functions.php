...
if ( !function_exists( 'issuem_leaky_paywall_new_subscriber' ) ) {
	 ...
	 * @return mixed $wpdb insert ID or false
	 */
	function issuem_leaky_paywall_new_subscriber( $hash, $email, $customer, $meta_args, $login='' ) {
		...
		if ( is_email( $email ) ) {
			...
			if ( !empty( $user_id ) ) {
				...
				do_action( 'issuem_leaky_paywall_new_subscriber', $user_id, $email, $meta, $customer, $meta_args );
...
if ( !function_exists( 'issuem_leaky_paywall_update_subscriber' ) ) {
	 ...
	 * @return mixed $wpdb insert ID or false
	 */
	function issuem_leaky_paywall_update_subscriber( $hash, $email, $customer, $meta_args ) {
		...
		if ( is_email( $email ) ) {
			...
			$meta = apply_filters( 'issuem_leaky_paywall_update_subscriber_meta', $meta, $email, $customer );
			...
			do_action( 'issuem_leaky_paywall_update_subscriber', $user_id, $email, $meta, $customer, $meta_args );
...
if ( !function_exists( 'issuem_leaky_paywall_subscriber_restrictions' ) ) {
	 ...
	 */
	function issuem_leaky_paywall_subscriber_restrictions() {
	
		$settings = get_issuem_leaky_paywall_settings();
		if ( false !== $restriction_level = issuem_leaky_paywall_susbscriber_current_level_id() ) {
...
if ( !function_exists( 'build_issuem_leaky_paywall_subscription_levels_row' ) ) {
	...
	function build_issuem_leaky_paywall_subscription_levels_row( $level=array(), $row_key='' ) {
		...
		$return .= apply_filters( 'build_issuem_leaky_paywall_subscription_levels_row_addon_filter', '', $level, $row_key );
...
if ( !function_exists( 'get_issuem_leaky_paywall_subscription_level' ) ) {
	
	function get_issuem_leaky_paywall_subscription_level( $level_id ) {
...
if ( !function_exists( 'issuem_leaky_paywall_process_stripe_payment' ) ) {
	
	function issuem_leaky_paywall_process_stripe_payment() {
		...
		if ( isset( $_POST['custom'] ) && !empty( $_POST['stripeToken'] ) ) {
			...
			try {
				...
				$level = get_issuem_leaky_paywall_subscription_level( $_POST['custom'] );
				...
				if ( !empty( $level['recurring'] ) && 'on' === $level['recurring'] && !empty( $level['plan_id'] ) ) {
					...
					$charge_array['currency']    = apply_filters( 'issuem_leaky_paywall_stripe_currency', 'usd' );
...
if ( !function_exists( 'issuem_leaky_paywall_process_paypal_payment' ) ) {
			...
			if ( !empty( $transaction_id ) && !empty( $transaction_amount ) && !empty( $transaction_status ) ) {
				...
				try {
					...
					if ( !is_wp_error( $response ) ) {
						...
						$level = get_issuem_leaky_paywall_subscription_level( $response_array['CUSTOM'] );
			...
			try {
				...
				if ( !is_object( $stripe_plan ) || //If we don't have a stripe plan
					...
					$args = array(
		                ...
		                'currency'          => esc_js( apply_filters( 'issuem_leaky_paywall_stripe_currency', 'usd' ) ),
		            ...
		            try {
			            ...
			            update_issuem_leaky_paywall_settings( $settings );
		            } catch( Exception $e ) {
