	...
	function leaky_paywall_translate_payment_gateway_slug_to_name($slug)
	{
		...
		switch ( $slug) {
			...
			case 'free_registration':
				$return = __( 'Free Registration', 'leak-paywall' );
				break;

			case 'manual':
				$return = __( 'Manually Added', 'leak-paywall' );
				break;
			default:
				$return = $slug;
				break;
	...
	function leaky_paywall_cancellation_confirmation()
	{
		...
		if ( is_user_logged_in() ) {
			...
			if ( ! empty( $_REQUEST['payment_gateway'] ) ) {
				...
				return '<p>' . __( 'No payment gateway defined.', 'leak-paywall' ) . '</p>';
			}

			if ( ! empty( $_REQUEST['subscriber_id'] ) ) {
				$subscriber_id = $_REQUEST['subscriber_id'];
			} else {
				return '<p>' . __( 'No subscriber ID defined.', 'leak-paywall' ) . '</p>';
			}

			if ( isset( $_REQUEST['cancel'] ) && empty( $_REQUEST['cancel'] ) ) {

				$form = '<h3>' . __( 'Cancel Subscription', 'leak-paywall' ) . '</h3>';

				$cancel_description = '<p>' . __( 'Cancellations take effect at the end of your billing cycle, and we can’t give partial refunds for unused time in the billing cycle. If you still wish to cancel now, you may proceed, or you can come back later.', 'leak-paywall' ) . '</p>';
				$cancel_description .= '<p>' . sprintf( __( ' Thank you for the time you’ve spent subscribed to %s. We hope you’ll return someday. ', 'leak-paywall' ), $settings['site_name'] ) . '</p>';

				$form .= apply_filters( 'leaky_paywall_cancel_subscription_description', $cancel_description);

				$form .= '<a href="' . esc_url(add_query_arg(array( 'cancel' => 'confirm' ) )) . '">' . __( 'Yes, cancel my subscription!', 'leak-paywall' ) . '</a> | <a href="' . get_page_link($settings['page_for_profile'] ) . '">' . __( 'No, get me outta here!', 'leak-paywall' ) . '</a>';
				...
				if ( 'stripe' === $payment_gateway) {
					...
					try {
						...
						if ( ! empty( $results->status ) && 'canceled' === $results->status ) {
							...
							$form .= '<p>' . sprintf( __( 'Your subscription has been successfully canceled. You will continue to have access to %s until the end of your billing cycle. Thank you for the time you have spent subscribed to our site and we hope you will return soon!', 'leak-paywall' ), $settings['site_name'] ) . '</p>';
							...
							$form .= '<p>' . sprintf( __( 'ERROR: An error occured when trying to unsubscribe you from your account, please try again. If you continue to have trouble, please contact us. Thank you.', 'leak-paywall' ), $settings['site_name'] ) . '</p>';
						}

						$form .= '<a href="' . get_home_url() . '">' . sprintf( __( 'Return to %s...', 'leak-paywall' ), $settings['site_name'] ) . '</a>';
					} catch (Exception $e) {

						$results = '<h1>' . sprintf( __( 'Error processing request: %s', 'leak-paywall' ), $e->getMessage()) . '</h1>';
					}
					...
					$form .= '<p>' . sprintf( __( 'You must cancel your account through PayPal. Please click this unsubscribe button to complete the cancellation process.', 'leak-paywall' ), $settings['site_name'] ) . '</p>';
					...
					$form .= '<p>' . __( 'Unable to determine your payment method. Please contact support for help canceling your account.', 'leak-paywall' ) . '</p>';
				}
			}
		} else {

			$form .= '<p>' . __( 'You must be logged in to cancel your account.', 'leak-paywall' ) . '</p>';
		}
...
if ( ! function_exists( 'send_leaky_paywall_email' ) ) {
	...
	function send_leaky_paywall_email( $email )
	{
		...
		return wp_mail( $email, __( 'Log into ' . get_bloginfo( 'name' ), 'leak-paywall' ), $message, $headers );
	}
}
		...
		if ( $response = wp_remote_head( str_replace( ' ', '%20', $url ) ) ) {
			if ( ! is_wp_error( $response ) ) {
				...
				if ( in_array( wp_remote_retrieve_response_code( $response ), (array) $valid_response_codes ) ) {
					...
					$output  = '<h3>' . __( 'Error Downloading PDF', 'leak-paywall' ) . '</h3>';
					$output .= '<p>' . sprintf( __( 'Download Error: Invalid response: %s', 'leak-paywall' ), wp_remote_retrieve_response_code( $response )) . '</p>';
				...
				$output  = '<h3>' . __( 'Error Downloading PDF', 'leak-paywall' ) . '</h3>';
				$output .= '<p>' . sprintf( __( 'Download Error: %s', 'leak-paywall' ), $response->get_error_message()) . '</p>';
				$output .= '<a href="' . get_home_url() . '">' . __( 'Home', 'leak-paywall' ) . '</a>';
...
if ( ! function_exists( 'build_leaky_paywall_subscription_levels_row' ) ) {
	...
	function build_leaky_paywall_subscription_levels_row( $level = array(), $row_key = '' )
	{
		...
		<table class="issuem-leaky-paywall-subscription-level-row-table leaky-paywall-table <?php echo $deleted; ?>">
			<tr>
				<th>
					<label for="level-name-<?php echo $row_key; ?>"><?php _e( 'Subscription Level Name', 'leak-paywall' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label for="level-description-<?php echo $row_key; ?>"><?php _e( 'Subscribe Card Description', 'leak-paywall' ); ?></label>
				</th>
				<td>
					...
					<p class="description"><?php _e( 'If entered, this will replace the auto-generated access description on the subscribe cards. HTML allowed.', 'leak-paywall' ); ?></p>
				</td>
			</tr>

			<tr>
				<th>
					<label for="level-registration-form-description-<?php echo $row_key; ?>"><?php _e( 'Registration Form Description', 'leak-paywall' ); ?></label>
				</th>
				<td>
					...
					<p class="description"><?php _e( 'If entered, this will replace the auto-generated content access description on the registration form. HTML allowed.', 'leak-paywall' ); ?></p>
				</td>
			</tr>
			...
			?>
				<tr>
					<th>
						<label for="level-recurring-<?php echo $row_key; ?>"><?php _e( 'Recurring', 'leak-paywall' ); ?></label>
					</th>
					<td>
			...
			<tr>
				<th>
					<label for="level-hide-subscribe-card-<?php echo $row_key; ?>"><?php _e( 'Hide Subscribe Card', 'leak-paywall' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label for="level-price-<?php echo $row_key; ?>"><?php _e( 'Subscription Price', 'leak-paywall' ); ?></label>
				</th>
				<td>
					...
					<p class="description"><?php _e( '0 for Free Subscriptions', 'leak-paywall' ); ?></p>
				</td>
			</tr>

			<tr>
				<th>
					<label for="level-interval-count-<?php echo $row_key; ?>"><?php _e( 'Subscription Length', 'leak-paywall' ); ?></label>
				</th>
				<td>
					<select class="subscription_length_type" name="levels[<?php echo $row_key; ?>][subscription_length_type]">
						<option value="unlimited" <?php echo selected( 'unlimited', $level['subscription_length_type'], false ); ?>><?php _e( 'Forever', 'leak-paywall' ); ?></option>
						<option value="limited" <?php echo selected( 'limited', $level['subscription_length_type'], false ); ?>> <?php _e( 'Limited for...', 'leak-paywall' ); ?></option>
					</select>
					...
					<div class="interval_div" style="<?php echo $subscription_length_input_style; ?>">
						...
						<select id="interval" name="levels[<?php echo $row_key; ?>][interval]">
							<option value="day" <?php echo selected( 'day' === $level['interval'], true, false ); ?>><?php _e( 'Day(s)', 'leak-paywall' ); ?></option>
							<option value="week" <?php echo selected( 'week' === $level['interval'], true, false ); ?>><?php _e( 'Week(s)', 'leak-paywall' ); ?></option>
							<option value="month" <?php echo selected( 'month' === $level['interval'], true, false ); ?>><?php _e( 'Month(s)', 'leak-paywall' ); ?></option>
							<option value="year" <?php echo selected( 'year' === $level['interval'], true, false ); ?>><?php _e( 'Year(s)', 'leak-paywall' ); ?></option>
						</select>
					</div>
				</td>
			</tr>
			...
			<tr>
				<th><?php _e( 'Access Options', 'leak-paywall' ); ?></th>
			...
			<tr>
				...
				<td>
					...
					<p><input data-row-key="<?php echo $row_key; ?>" class="button-secondary" id="add-subscription-row-post-type" class="add-new-issuem-leaky-paywall-row-post-type" type="submit" name="add_leaky_paywall_subscription_row_post_type" value="<?php _e( '+ Add Access Option', 'leak-paywall' ); ?>" /></p>
					<?php
					if ( $leaky_paywall->is_site_wide_enabled() ) {
						echo '<p class="description">';
						_e( 'Post Types that are not native the to the site currently being viewed are marked with an asterisk.', 'leak-paywall' );
						echo '</p>';
					}
					?>

					<p class="description"><?php _e( 'Access processed from top to bottom.', 'leak-paywall' ); ?></p>
				</td>
			</tr>
			...
			?>
				<tr>
					<th><?php _e( 'Site', 'leak-paywall' ); ?></th>
					<td id="issuem-leaky-paywall-subsciption-row-<?php echo $row_key; ?>-site">
						<select id="site" name="levels[<?php echo $row_key; ?>][site]">
							<?php
							if ( is_super_admin() ) {
							?>
								<option value="all" <?php echo selected( 'all', $level['site'], false ); ?>><?php _e( 'All Sites', 'leak-paywall' ); ?></option>
								<?php
								$sites = get_sites();
								foreach ( $sites as $site ) {
	...
	if ( ! function_exists( 'build_leaky_paywall_subscription_row_post_type' ) ) {
		...
		function build_leaky_paywall_subscription_row_post_type( $select_post_type = array(), $select_post_key = '', $row_key = '' )
		{
			...
			echo '<option value="unlimited" ' . selected( 'unlimited', $select_post_type['allowed'], false ) . '>' . __( 'Unlimited', 'leak-paywall' ) . '</option>';
			echo '<option value="limited" ' . selected( 'limited', $select_post_type['allowed'], false ) . '>' . __( 'Limit to...', 'leak-paywall' ) . '</option>';
			echo '</select>';
			...
			echo '<input type="number" class="allowed_value small-text" name="levels[' . $row_key . '][post_types][' . $select_post_key . '][allowed_value]" value="' . $select_post_type['allowed_value'] . '" placeholder="' . __( '#', 'leak-paywall' ) . '" />';
			echo '</div></td>';
	...
	if ( ! function_exists( 'leaky_paywall_subscription_options' ) ) {
		...
		function leaky_paywall_subscription_options()
		{
			...
			if ( empty( $results ) ) {
				...
				$results .= '<h2 class="subscription-options-title">' . __( 'Subscription Options', 'leak-paywall' ) . '</h2>';
					...
					foreach ( apply_filters( 'leaky_paywall_subscription_levels', $settings['levels'] ) as $level_id => $level ) {
								...
								if ( isset( $post_type['taxonomy'] ) ) {
									...
									if ( ! empty( $post_type_obj ) ) {
										if ( 0 <= $post_type['allowed_value'] ) {
											...
											$allowed_content .= '<p>'  . sprintf( __( 'Access %s %s %s*', 'leak-paywall' ), $post_type['allowed_value'], $name, $post_type_obj->labels->singular_name . $plural) .  '</p>';
										} else {
											$allowed_content .= '<p>' . sprintf( __( 'Unlimited %s %s', 'leak-paywall' ), $name, $post_type_obj->labels->name) . '</p>';
										}
									}
								} else {
									...
									if ( ! empty( $post_type_obj ) ) {
										if (0 <= $post_type['allowed_value'] ) {
											...
											$allowed_content .= '<p>'  . sprintf( __( 'Access %s %s*', 'leak-paywall' ), $post_type['allowed_value'], $post_type_obj->labels->singular_name . $plural) .  '</p>';
										} else {
											$allowed_content .= '<p>' . sprintf( __( 'Unlimited %s', 'leak-paywall' ), $post_type_obj->labels->name) . '</p>';
										}
									}
								}
							}
						...
						if ( ! empty( $level['price'] ) ) {
							if ( ! empty( $level['recurring'] ) && 'on' === $level['recurring'] && apply_filters( 'leaky_paywall_subscription_options_price_recurring_on', true, $current_level) ) {
								$subscription_price .= '<strong>' . leaky_paywall_get_level_display_price( $level ) . ' ' . leaky_paywall_human_readable_interval($level['interval_count'], $level['interval'] ) . ' ' . __( '(recurring)', 'leak-paywall' ) . '</strong>';
								$subscription_price .= apply_filters( 'leaky_paywall_before_subscription_options_recurring_price', '' );
							} else {
								$subscription_price .= '<strong>' . sprintf( __( '%s %s', 'leak-paywall' ), leaky_paywall_get_level_display_price( $level ), leaky_paywall_human_readable_interval($level['interval_count'], $level['interval'] )) . '</strong>';
							...
							if ( ! empty( $level['trial_period'] ) ) {
								$subscription_price .= '<span class="leaky-paywall-trial-period">' . sprintf( __( 'Free for the first %s day(s)', 'leak-paywall' ), $level['trial_period'] ) . '</span>';
							}
						} else {
							$subscription_price .= '<strong>' . __( 'Free', 'leak-paywall' ) . '</strong>';
						}
						...
						if ( in_array( $level_id, $current_level_ids ) ) {
							...
							$subscription_action .= __( 'Your Current Subscription', 'leak-paywall' );
	...
	if ( ! function_exists( 'leaky_paywall_add_lost_password_link' ) ) {
		...
		function leaky_paywall_add_lost_password_link()
		{
			...
			$settings = get_leaky_paywall_settings();
			return '<a href="' . wp_lostpassword_url() . '">' . __( 'Lost Password?', 'leak-paywall' ) . '</a>';
		}
	}
	...
	if ( ! function_exists( 'leaky_paywall_payment_gateways' ) ) {
		...
		{
			$gateways = array(
				'manual'            => __( 'Manual', 'leak-paywall' ),
				'stripe'            => __( 'Stripe', 'leak-paywall' ),
				'paypal_standard'   => __( 'PayPal Standard', 'leak-paywall' ),
				'free_registration' => __( 'Free Registration', 'leak-paywall' ),
			);
	...
	if ( ! function_exists( 'leaky_paywall_human_readable_interval' ) ) {
		function leaky_paywall_human_readable_interval( $interval_count, $interval )
		{
			...
			if ( 0 >= $interval_count )
				return __( 'for life', 'leak-paywall' );
			...
			switch ( $interval ) {
				case 'day':
					$interval_str = __( 'day', 'leak-paywall' );
					break;
				case 'days':
					$interval_str = __( 'days', 'leak-paywall' );
					break;
				case 'week':
					$interval_str = __( 'week', 'leak-paywall' );
					break;
				case 'weeks':
					$interval_str = __( 'weeks', 'leak-paywall' );
					break;
				case 'month':
					$interval_str = __( 'month', 'leak-paywall' );
					break;
				case 'months':
					$interval_str = __( 'months', 'leak-paywall' );
					break;
				case 'year':
					$interval_str = __( 'year', 'leak-paywall' );
					break;
				case 'years':
					$interval_str = __( 'years', 'leak-paywall' );
					break;
				default:
			...
			if ( 1 == $interval_count )
				return __( 'every', 'leak-paywall' ) . ' ' . $interval_str;
			else
				return __( 'every', 'leak-paywall' ) . ' ' . $interval_count . ' ' . $interval_str;
		}
	}
	...
	if ( ! function_exists( 'leaky_paywall_supported_currencies' ) ) {
		...
		{
			$currencies = array(
				'AED' => array( 'symbol' => '&#1583;.&#1573;', 'label' => __( 'UAE dirham', 'leak-paywall' ), 'country' => __( 'UAE', 'leaky-paywall' ) ),
				'AFN' => array( 'symbol' => 'Afs', 'label' => __( 'Afghan afghani', 'leak-paywall' ), 'country' => __( 'Afghanistan', 'leaky-paywall' ) ),
				'ALL' => array( 'symbol' => 'L', 'label' => __( 'Albanian lek', 'leak-paywall' ), 'country' => __( 'Albania', 'leaky-paywall' ) ),
				'AMD' => array( 'symbol' => 'AMD', 'label' => __( 'Armenian dram', 'leak-paywall' ), 'country' => __( 'Armenia', 'leaky-paywall' ) ),
				'ANG' => array( 'symbol' => 'NA&#402;', 'label' => __( 'Netherlands Antillean gulden', 'leak-paywall' ), 'country' => __( 'Netherlands', 'leaky-paywall' ) ),
				'AOA' => array( 'symbol' => 'Kz', 'label' => __( 'Angolan kwanza', 'leak-paywall' ), 'country' => __( 'Angolia', 'leaky-paywall' ) ),
				'ARS' => array( 'symbol' => '$', 'label' => __( 'Argentine peso', 'leak-paywall' ), 'country' => __( 'Argentina', 'leaky-paywall' ) ),
				'AUD' => array( 'symbol' => '$', 'label' => __( 'Australian dollar', 'leak-paywall' ), 'country' => __( 'Australia', 'leaky-paywall' ) ),
				'AWG' => array( 'symbol' => '&#402;', 'label' => __( 'Aruban florin', 'leak-paywall' ), 'country' => __( 'Aruba', 'leaky-paywall' ) ),
				'AZN' => array( 'symbol' => 'AZN', 'label' => __( 'Azerbaijani manat', 'leak-paywall' ), 'country' => __( 'Azerbaij', 'leaky-paywall' ) ),
				'BAM' => array( 'symbol' => 'KM', 'label' => __( 'Bosnia and Herzegovina konvertibilna marka', 'leak-paywall' ), 'country' => __( 'Bosnia', 'leaky-paywall' ) ),
				'BBD' => array( 'symbol' => 'Bds$', 'label' => __( 'Barbadian dollar', 'leak-paywall' ), 'country' => __( 'Barbadian', 'leaky-paywall' ) ),
				'BDT' => array( 'symbol' => '&#2547;', 'label' => __( 'Bangladeshi taka', 'leak-paywall' ), 'country' => __( 'Bangladesh', 'leaky-paywall' ) ),
				'BGN' => array( 'symbol' => 'BGN', 'label' => __( 'Bulgarian lev', 'leak-paywall' ), 'country' => __( 'Bulgaria', 'leaky-paywall' ) ),
				'BIF' => array( 'symbol' => 'FBu', 'label' => __( 'Burundi franc', 'leak-paywall' ), 'country' => __( 'Burundi', 'leaky-paywall' ) ),
				'BMD' => array( 'symbol' => 'BD$', 'label' => __( 'Bermudian dollar', 'leak-paywall' ), 'country' => __( 'Bermuda', 'leaky-paywall' ) ),
				'BND' => array( 'symbol' => 'B$', 'label' => __( 'Brunei dollar', 'leak-paywall' ), 'country' => __( 'Brunei', 'leaky-paywall' ) ),
				'BOB' => array( 'symbol' => 'Bs.', 'label' => __( 'Bolivian boliviano', 'leak-paywall' ), 'country' => __( 'Bolivia', 'leaky-paywall' ) ),
				'BRL' => array( 'symbol' => 'R$', 'label' => __( 'Brazilian real', 'leak-paywall' ), 'country' => __( 'Brazil', 'leaky-paywall' ) ),
				'BSD' => array( 'symbol' => 'B$', 'label' => __( 'Bahamian dollar', 'leak-paywall' ), 'country' => __( 'Bahamas', 'leaky-paywall' ) ),
				'BWP' => array( 'symbol' => 'P', 'label' => __( 'Botswana pula', 'leak-paywall' ), 'country' => __( 'Botswana', 'leaky-paywall' ) ),
				'BZD' => array( 'symbol' => 'BZ$', 'label' => __( 'Belize dollar', 'leak-paywall' ), 'country' => __( 'Belize', 'leaky-paywall' ) ),
				'CAD' => array( 'symbol' => '$', 'label' => __( 'Canadian dollar', 'leak-paywall' ), 'country' => __( 'Canada', 'leaky-paywall' ) ),
				'CDF' => array( 'symbol' => 'F', 'label' => __( 'Congolese franc', 'leak-paywall' ), 'country' => __( 'Congo', 'leaky-paywall' ) ),
				'CHF' => array( 'symbol' => 'CHF', 'label' => __( 'Swiss franc', 'leak-paywall' ), 'country' => __( 'Switzerland', 'leaky-paywall' ) ),
				'CLP' => array( 'symbol' => '$', 'label' => __( 'Chilean peso', 'leak-paywall' ), 'country' => __( 'Chili', 'leaky-paywall' ) ),
				'CNY' => array( 'symbol' => '&#165;', 'label' => __( 'Chinese Yuan Renminbi', 'leak-paywall' ), 'country' => __( 'Chinese Yuan', 'leaky-paywall' ) ),
				'COP' => array( 'symbol' => 'Col$', 'label' => __( 'Colombian peso', 'leak-paywall' ), 'country' => __( 'Colombia', 'leaky-paywall' ) ),
				'CRC' => array( 'symbol' => '&#8353;', 'label' => __( 'Costa Rican colon', 'leak-paywall' ), 'country' => __( 'Costa Rica', 'leaky-paywall' ) ),
				'CVE' => array( 'symbol' => 'Esc', 'label' => __( 'Cape Verdean escudo', 'leak-paywall' ), 'country' => __( 'Cape Verde', 'leaky-paywall' ) ),
				'CZK' => array( 'symbol' => 'K&#269;', 'label' => __( 'Czech koruna', 'leak-paywall' ), 'country' => __( 'Czech', 'leaky-paywall' ) ),
				'DJF' => array( 'symbol' => 'Fdj', 'label' => __( 'Djiboutian franc', 'leak-paywall' ), 'country' => __( 'Djibouti', 'leaky-paywall' ) ),
				'DKK' => array( 'symbol' => 'kr', 'label' => __( 'Danish krone', 'leak-paywall' ), 'country' => __( 'Danish', 'leaky-paywall' ) ),
				'DOP' => array( 'symbol' => 'RD$', 'label' => __( 'Dominican peso', 'leak-paywall' ), 'country' => __( 'Dominican Republic', 'leaky-paywall' ) ),
				'DZD' => array( 'symbol' => '&#1583;.&#1580;', 'label' => __( 'Algerian dinar', 'leak-paywall' ), 'country' => __( 'Algeria', 'leaky-paywall' ) ),
				'EEK' => array( 'symbol' => 'KR', 'label' => __( 'Estonian kroon', 'leak-paywall' ), 'country' => __( 'Estonia', 'leaky-paywall' ) ),
				'EGP' => array( 'symbol' => '&#163;', 'label' => __( 'Egyptian pound', 'leak-paywall' ), 'country' => __( 'Egypt', 'leaky-paywall' ) ),
				'ETB' => array( 'symbol' => 'Br', 'label' => __( 'Ethiopian birr', 'leak-paywall' ), 'country' => __( 'Ethiopia', 'leaky-paywall' ) ),
				'EUR' => array( 'symbol' => '&#8364;', 'label' => __( 'European Euro', 'leak-paywall' ), 'country' => __( 'Euro', 'leaky-paywall' ) ),
				'FJD' => array( 'symbol' => 'FJ$', 'label' => __( 'Fijian dollar', 'leak-paywall' ), 'country' => __( 'Fiji', 'leaky-paywall' ) ),
				'FKP' => array( 'symbol' => '&#163;', 'label' => __( 'Falkland Islands pound', 'leak-paywall' ), 'country' => __( 'Falkland Islands', 'leaky-paywall' ) ),
				'GBP' => array( 'symbol' => '&#163;', 'label' => __( 'British pound', 'leak-paywall' ), 'country' => __( 'Great Britian', 'leaky-paywall' ) ),
				'GEL' => array( 'symbol' => 'GEL', 'label' => __( 'Georgian lari', 'leak-paywall' ), 'country' => __( 'Georgia', 'leaky-paywall' ) ),
				'GIP' => array( 'symbol' => '&#163;', 'label' => __( 'Gibraltar pound', 'leak-paywall' ), 'country' => __( 'Gibraltar', 'leaky-paywall' ) ),
				'GMD' => array( 'symbol' => 'D', 'label' => __( 'Gambian dalasi', 'leak-paywall' ), 'country' => __( 'Gambia', 'leaky-paywall' ) ),
				'GNF' => array( 'symbol' => 'FG', 'label' => __( 'Guinean franc', 'leak-paywall' ), 'country' => __( 'Guinea', 'leaky-paywall' ) ),
				'GTQ' => array( 'symbol' => 'Q', 'label' => __( 'Guatemalan quetzal', 'leak-paywall' ), 'country' => __( 'Guatemala', 'leaky-paywall' ) ),
				'GYD' => array( 'symbol' => 'GY$', 'label' => __( 'Guyanese dollar', 'leak-paywall' ), 'country' => __( 'Guyanese', 'leaky-paywall' ) ),
				'HKD' => array( 'symbol' => 'HK$', 'label' => __( 'Hong Kong dollar', 'leak-paywall' ), 'country' => __( 'Hong Kong', 'leaky-paywall' ) ),
				'HNL' => array( 'symbol' => 'L', 'label' => __( 'Honduran lempira', 'leak-paywall' ), 'country' => __( 'Honduras', 'leaky-paywall' ) ),
				'HRK' => array( 'symbol' => 'kn', 'label' => __( 'Croatian kuna', 'leak-paywall' ), 'country' => __( 'Croatia', 'leaky-paywall' ) ),
				'HTG' => array( 'symbol' => 'G', 'label' => __( 'Haitian gourde', 'leak-paywall' ), 'country' => __( 'Haiti', 'leaky-paywall' ) ),
				'HUF' => array( 'symbol' => 'Ft', 'label' => __( 'Hungarian forint', 'leak-paywall' ), 'country' => __( 'Hungary', 'leaky-paywall' ) ),
				'IDR' => array( 'symbol' => 'Rp', 'label' => __( 'Indonesian rupiah', 'leak-paywall' ), 'country' => __( 'Idonesia', 'leaky-paywall' ) ),
				'ILS' => array( 'symbol' => '&#8362;', 'label' => __( 'Israeli new sheqel', 'leak-paywall' ), 'country' => __( 'Israel', 'leaky-paywall' ) ),
				'INR' => array( 'symbol' => '&#8377;', 'label' => __( 'Indian rupee', 'leak-paywall' ), 'country' => __( 'India', 'leaky-paywall' ) ),
				'ISK' => array( 'symbol' => 'kr', 'label' => __( 'Icelandic króna', 'leak-paywall' ), 'country' => __( 'Iceland', 'leaky-paywall' ) ),
				'JMD' => array( 'symbol' => 'J$', 'label' => __( 'Jamaican dollar', 'leak-paywall' ), 'country' => __( 'Jamaica', 'leaky-paywall' ) ),
				'JPY' => array( 'symbol' => '&#165;', 'label' => __( 'Japanese yen', 'leak-paywall' ), 'country' => __( 'Japan', 'leaky-paywall' ) ),
				'KES' => array( 'symbol' => 'KSh', 'label' => __( 'Kenyan shilling', 'leak-paywall' ), 'country' => __( 'Kenya', 'leaky-paywall' ) ),
				'KGS' => array( 'symbol' => '&#1089;&#1086;&#1084;', 'label' => __( 'Kyrgyzstani som', 'leak-paywall' ), 'country' => __( 'Kyrgyzstan', 'leaky-paywall' ) ),
				'KHR' => array( 'symbol' => '&#6107;', 'label' => __( 'Cambodian riel', 'leak-paywall' ), 'country' => __( 'Cambodia', 'leaky-paywall' ) ),
				'KMF' => array( 'symbol' => 'KMF', 'label' => __( 'Comorian franc', 'leak-paywall' ), 'country' => __( 'Comorian', 'leaky-paywall' ) ),
				'KRW' => array( 'symbol' => 'W', 'label' => __( 'South Korean won', 'leak-paywall' ), 'country' => __( 'South Korea', 'leaky-paywall' ) ),
				'KYD' => array( 'symbol' => 'KY$', 'label' => __( 'Cayman Islands dollar', 'leak-paywall' ), 'country' => __( 'Cayman Islands', 'leaky-paywall' ) ),
				'KZT' => array( 'symbol' => 'T', 'label' => __( 'Kazakhstani tenge', 'leak-paywall' ), 'country' => __( 'Kazakhstan', 'leaky-paywall' ) ),
				'LAK' => array( 'symbol' => 'KN', 'label' => __( 'Lao kip', 'leak-paywall' ), 'country' => __( 'Loa', 'leaky-paywall' ) ),
				'LBP' => array( 'symbol' => '&#163;', 'label' => __( 'Lebanese lira', 'leak-paywall' ), 'country' => __( 'Lebanese', 'leaky-paywall' ) ),
				'LKR' => array( 'symbol' => 'Rs', 'label' => __( 'Sri Lankan rupee', 'leak-paywall' ), 'country' => __( 'Sri Lanka', 'leaky-paywall' ) ),
				'LRD' => array( 'symbol' => 'L$', 'label' => __( 'Liberian dollar', 'leak-paywall' ), 'country' => __( 'Liberia', 'leaky-paywall' ) ),
				'LSL' => array( 'symbol' => 'M', 'label' => __( 'Lesotho loti', 'leak-paywall' ), 'country' => __( 'Lesotho', 'leaky-paywall' ) ),
				'LTL' => array( 'symbol' => 'Lt', 'label' => __( 'Lithuanian litas', 'leak-paywall' ), 'country' => __( 'Lithuania', 'leaky-paywall' ) ),
				'LVL' => array( 'symbol' => 'Ls', 'label' => __( 'Latvian lats', 'leak-paywall' ), 'country' => __( 'Latvia', 'leaky-paywall' ) ),
				'MAD' => array( 'symbol' => 'MAD', 'label' => __( 'Moroccan dirham', 'leak-paywall' ), 'country' => __( 'Morocco', 'leaky-paywall' ) ),
				'MDL' => array( 'symbol' => 'MDL', 'label' => __( 'Moldovan leu', 'leak-paywall' ), 'country' => __( 'Moldova', 'leaky-paywall' ) ),
				'MGA' => array( 'symbol' => 'FMG', 'label' => __( 'Malagasy ariary', 'leak-paywall' ), 'country' => __( 'Malagasy', 'leaky-paywall' ) ),
				'MKD' => array( 'symbol' => 'MKD', 'label' => __( 'Macedonian denar', 'leak-paywall' ), 'country' => __( 'Macedonia', 'leaky-paywall' ) ),
				'MNT' => array( 'symbol' => '&#8366;', 'label' => __( 'Mongolian tugrik', 'leak-paywall' ), 'country' => __( 'Mongolia', 'leaky-paywall' ) ),
				'MOP' => array( 'symbol' => 'P', 'label' => __( 'Macanese pataca', 'leak-paywall' ), 'country' => __( 'Macanese', 'leaky-paywall' ) ),
				'MRO' => array( 'symbol' => 'UM', 'label' => __( 'Mauritanian ouguiya', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'MUR' => array( 'symbol' => 'Rs', 'label' => __( 'Mauritian rupee', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'MVR' => array( 'symbol' => 'Rf', 'label' => __( 'Maldivian rufiyaa', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'MWK' => array( 'symbol' => 'MK', 'label' => __( 'Malawian kwacha', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'MXN' => array( 'symbol' => '$', 'label' => __( 'Mexican peso', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'MYR' => array( 'symbol' => 'RM', 'label' => __( 'Malaysian ringgit', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'MZN' => array( 'symbol' => 'MT', 'label' => __( 'Mozambique Metical', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'NAD' => array( 'symbol' => 'N$', 'label' => __( 'Namibian dollar', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'NGN' => array( 'symbol' => '&#8358;', 'label' => __( 'Nigerian naira', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'NIO' => array( 'symbol' => 'C$', 'label' => __( 'Nicaraguan Córdoba', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'NOK' => array( 'symbol' => 'kr', 'label' => __( 'Norwegian krone', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'NPR' => array( 'symbol' => 'NRs', 'label' => __( 'Nepalese rupee', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'NZD' => array( 'symbol' => 'NZ$', 'label' => __( 'New Zealand dollar', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'PAB' => array( 'symbol' => 'B./', 'label' => __( 'Panamanian balboa', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'PEN' => array( 'symbol' => 'S/.', 'label' => __( 'Peruvian nuevo sol', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'PGK' => array( 'symbol' => 'K', 'label' => __( 'Papua New Guinean kina', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'PHP' => array( 'symbol' => '&#8369;', 'label' => __( 'Philippine peso', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'PKR' => array( 'symbol' => 'Rs.', 'label' => __( 'Pakistani rupee', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'PLN' => array( 'symbol' => 'z&#322;', 'label' => __( 'Polish zloty', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'PYG' => array( 'symbol' => '&#8370;', 'label' => __( 'Paraguayan guarani', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'QAR' => array( 'symbol' => 'QR', 'label' => __( 'Qatari riyal', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'RON' => array( 'symbol' => 'L', 'label' => __( 'Romanian leu', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'RSD' => array( 'symbol' => 'din.', 'label' => __( 'Serbian dinar', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'RUB' => array( 'symbol' => 'R', 'label' => __( 'Russian ruble', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'RWF' => array( 'symbol' => 'R&#8355;', 'label' => __( 'Rwandan Franc'), 'country' => __( '', 'leaky-paywall' ) ),
				'SAR' => array( 'symbol' => 'SR', 'label' => __( 'Saudi riyal', 'leaky-paywall' ) ),
				'SBD' => array( 'symbol' => 'SI$', 'label' => __( 'Solomon Islands dollar', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'SCR' => array( 'symbol' => 'SR', 'label' => __( 'Seychellois rupee', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'SEK' => array( 'symbol' => 'kr', 'label' => __( 'Swedish krona', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'SGD' => array( 'symbol' => 'S$', 'label' => __( 'Singapore dollar', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'SHP' => array( 'symbol' => '&#163;', 'label' => __( 'Saint Helena pound', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'SLL' => array( 'symbol' => 'Le', 'label' => __( 'Sierra Leonean leone', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'SOS' => array( 'symbol' => 'Sh.', 'label' => __( 'Somali shilling', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'SRD' => array( 'symbol' => '$', 'label' => __( 'Surinamese dollar', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'STD' => array( 'symbol' => 'STD', 'label' => __( 'São Tomé and Príncipe Dobra', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'SVC' => array( 'symbol' => '$', 'label' => __( 'El Salvador Colon', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'SZL' => array( 'symbol' => 'E', 'label' => __( 'Swazi lilangeni', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'THB' => array( 'symbol' => '&#3647;', 'label' => __( 'Thai baht', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'TJS' => array( 'symbol' => 'TJS', 'label' => __( 'Tajikistani somoni', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'TOP' => array( 'symbol' => 'T$', 'label' => __("Tonga Pa'anga", 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'TRY' => array( 'symbol' => 'TRY', 'label' => __( 'Turkish new lira', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'TTD' => array( 'symbol' => 'TT$', 'label' => __( 'Trinidad and Tobago dollar', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'TWD' => array( 'symbol' => 'NT$', 'label' => __( 'New Taiwan dollar', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'TZS' => array( 'symbol' => 'TZS', 'label' => __( 'Tanzanian shilling', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'UAH' => array( 'symbol' => 'UAH', 'label' => __( 'Ukrainian hryvnia', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'UGX' => array( 'symbol' => 'USh', 'label' => __( 'Ugandan shilling', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'USD' => array( 'symbol' => '$', 'label' => __( 'United States dollar', 'leak-paywall' ), 'country' => __( 'United States', 'leaky-paywall' ) ),
				'UYU' => array( 'symbol' => '$U', 'label' => __( 'Uruguayan peso', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'UZS' => array( 'symbol' => 'UZS', 'label' => __( 'Uzbekistani som', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'VND' => array( 'symbol' => '&#8363;', 'label' => __( 'Vietnamese dong', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'VUV' => array( 'symbol' => 'VT', 'label' => __( 'Vanuatu vatu', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'WST' => array( 'symbol' => 'WS$', 'label' => __( 'Samoan tala', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'XAF' => array( 'symbol' => 'CFA', 'label' => __( 'Central African CFA franc', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'XCD' => array( 'symbol' => 'EC$', 'label' => __( 'East Caribbean dollar', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'XOF' => array( 'symbol' => 'CFA', 'label' => __( 'West African CFA franc', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'XPF' => array( 'symbol' => 'F', 'label' => __( 'CFP franc', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'YER' => array( 'symbol' => 'YER', 'label' => __( 'Yemeni rial', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'ZAR' => array( 'symbol' => 'R', 'label' => __( 'South African rand', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
				'ZMW' => array( 'symbol' => 'ZK', 'label' => __( 'Zambian kwacha', 'leak-paywall' ), 'country' => __( '', 'leaky-paywall' ) ),
			);
	...
	function leaky_paywall_plugin_row_meta( $input, $file )
	{
		...
		$links = array(
			'<a href="' . $lp_link . '">' . esc_html__( 'Add-Ons', 'leak-paywall' ) . '</a>',
		);
	...
	function leaky_paywall_maybe_delete_user()
	{
		...
		if ( in_array( 'subscriber', $user->roles ) ) {
			...
			wp_mail( $admin_emails, sprintf( __( 'User Account Deleted on %s', 'leak-paywall' ), $site_name), $admin_message, $headers );
	...
	function leaky_paywall_get_level_display_price( $level )
	{
		...
		if ( $price > 0 ) {
			...
			$display_price = __( 'Free', 'leak-paywall' );
		}
		...
		<div class="notice updated is-dismissible leaky-paywall-message leaky-paywall-message-dismissed" data-notice_id="<?php echo esc_attr( $notice_id); ?>">
			<div class="leaky-paywall-message-inner">
				...
				<div class="leaky-paywall-message-content">
					<p><strong><?php echo __( 'Congrats!', 'leak-paywall' ); ?></strong> 🥳<?php _e( 'You have more than 100 subscribers with <strong>Leaky Paywall</strong>. Please help us by leaving a review on WordPress.org. <br>We read every review and use your feedback to make Leaky Paywall better for everyone!', 'leak-paywall' ); ?></p>
					<p class="leaky-paywall-message-actions">
						<a href="https://wordpress.org/support/plugin/leaky-paywall/reviews/?filter=5/#new-post" target="_blank" class="button button-primary"><?php _e( 'Leave a Review', 'leak-paywall' ); ?></a>
						<a href="<?php echo esc_url_raw( $dismiss_url ); ?>" class="button leaky-paywall-button-notice-dismiss"><?php _e( 'Hide', 'leak-paywall' ); ?></a>
					</p>
				</div>
